...
public class UpdateDonActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        editTextPickUpTime.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                timePickerDialog = new TimePickerDialog(UpdateDonActivity.this, new TimePickerDialog.OnTimeSetListener() {
                    @Override
        ...
        getSupportActionBar().setTitle("Edit Your Donation");
   ...
   public void dataDon(){
      String title = getIntent().getExtras().getString("Tilte") ;
      ...
      String time = getIntent().getExtras().getString("Time");
       codeImg = getIntent().getExtras().getString("Img");

      ArrayAdapter adapterNbr = (ArrayAdapter) spinnerNumber.getAdapter() ;
      int positionItem = adapterNbr.getPosition(nbPerson);

      if(codeImg != null) {
          byte[] decodedString = Base64.decode(codeImg, Base64.DEFAULT);
       ...
       spinnerNumber.setSelection(positionItem);
    ...
    public void updateFood() {
        ...
        if ((!TextUtils.isEmpty(title)) && (!TextUtils.isEmpty(timepickup))) {
            ...
            if (isOnline()) {
                // Toast.makeText(UpdateDonActivity.this, "picture !!!", Toast.LENGTH_SHORT).show();

                if((imageBitmap == null)&&(codeImg == null)) {
                    Toast.makeText(UpdateDonActivity.this, "Your photo not added  " , LENGTH_LONG).show();
                    ...
                    //  Toast.makeText(UpdateDonActivity.this, "You are connected to Internet " , LENGTH_LONG).show();
                ...
                if((codeImg != null)&&(imageBitmap == null)){
                    don = new Don(id, editTextTitle.getText().toString(), editTextDescription.getText().toString(), editTextHealthCondition.getText().toString(), editTextAdr.getText().toString(), itemNumberPerson, editTextPickUpTime.getText().toString(), switchState, codeImg);
                }
                reference.child(id).setValue(don).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(UpdateDonActivity.this, "Don Food it's saved with success ", LENGTH_LONG).show();
                            Intent intent = new Intent(UpdateDonActivity.this, ProfileDonater.class);
                            startActivity(intent);

                        } else {
                            //  Log.d(TAG, task.getException().getMessage());
                            Toast.makeText(UpdateDonActivity.this, "Don Food it's not saved  ", LENGTH_LONG).show();
                        }
                    }
                });
            }else{

                Toast.makeText(UpdateDonActivity.this, "You are not connected to Internet", Toast.LENGTH_SHORT).show();
            }
        } else {
           ...
           /* if(imageBitmap == null){
                    ...
                    public void onChildChanged(@NonNull DataSnapshot dataSnapshot, @Nullable String s) {
                        ...
                        for (DataSnapshot donSnapshot : dataSnapshot.getChildren()) {
                            ...
                            if (donSnapshot.child(id).getValue() != null) {
                                Toast.makeText(UpdateDonActivity.this, "Don Food it's saved with success "  + switchState, LENGTH_LONG).show();
                               *//* Intent intent = new Intent(UpdateDonActivity.this, FragmentDonation.class);
                                startActivity(intent);*//*
                            } else {
                                Toast.makeText(UpdateDonActivity.this, "Don Food it's not saved  " , LENGTH_LONG).show();
                            }
            ...
            /*} else {
                Toast.makeText(UpdateDonActivity.this, "its saved without picture !", Toast.LENGTH_SHORT).show();
            }*/
    ...
    private void requestCameraAndStoragePermission() {
        ActivityCompat.requestPermissions(UpdateDonActivity.this, new String[]{Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 0);
    }

    private boolean hasCameraPermission() {
        return ContextCompat.checkSelfPermission(UpdateDonActivity.this, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED;
    }
